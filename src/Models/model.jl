...
function replaceshape!(expr, varnum=0, shpnum=0)
    ...
    if (Meta.isexpr(expr, :call) && Meta.isoperator(expr.args[1])
        && hastype(expr, Shape, ["delta", "gauss"]))
        for j=2:length(expr.args)
            arg = expr.args[j]
            println("shape   :", string(arg))
            if any(x->startswith(string(arg),x), ["delta", "gauss"])
                varnum += startswith(string(arg),"delta") ? 2 : 5
                shpnum += 1
...
function replace!(expr, varnum=0, shpnum=0)
    ...
    if Meta.isexpr(expr, :call)
        ...
        elseif (!Meta.isoperator(arg) && (arg in [:(delta), :(gauss)]
                                          || typeof(eval(arg)) <: Shape))
            varnum += arg == :(delta) ? 2 : 5
            shpnum += 1
